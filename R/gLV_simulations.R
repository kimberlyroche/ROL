...
#' Helper function for hockeystick plot that calculates a 2D summary of a taxon-taxon association
#' across hosts
#' 
#' @param heatmap vector or matrix (rows as hosts) of taxon-taxon correlations across hosts
#' @return NULL
#' @export
summarize_pairs_2D <- function(heatmap) {
  if(is.null(dim(heatmap))) {
    dim(heatmap) <- c(length(heatmap), 1)
  }
  signed_pairs <- sign(heatmap)
  proportion_shared_direction <- apply(signed_pairs, 2, function(z) {
    max(table(z)/length(z))
  })
  mean_strength_correlation <- apply(heatmap, 2, function(z) {
    mean(abs(z))
  })
  list(x = proportion_shared_direction, y = mean_strength_correlation)
}

#' Plot a gLV simulation as a 2D summary (hockeystick)
#' 
#' @param heatmap matrix of correlations (rows are hosts, columns are taxon-taxon pairs)
#' @return NULL
#' @import driver
#' @import ggplot2
#' @export
plot_gLV_hockeystick <- function(heatmap) {
  points <- summarize_pairs_2D(heatmap)
  df <- data.frame(x = points$x, y = points$y)
  ggplot(df) +
...
#' 
#' @param heatmap1 first matrix of correlations (rows are hosts, columns are taxon-taxon pairs)
#' @param heatmap2 second matrix of correlations (rows are hosts, columns are taxon-taxon pairs)
#' @return NULL
#' @import emdist
#' @export
calculate_emd <- function(heatmap1, heatmap2) {
  points1 <- summarize_pairs_2D(heatmap1)
  points2 <- summarize_pairs_2D(heatmap2)
  
  # Bin the 2D summaries
  binned_x <- unname(table(cut(points1$x, breaks = seq(0.5, 1, length.out = 20))))
  binned_y <- unname(table(cut(points1$y, breaks = seq(0, 1, length.out = 20))))
  binned_data1 <- rbind(binned_x, binned_y)
  
  binned_x <- unname(table(cut(points2$x, breaks = seq(0.5, 1, length.out = 20))))
  binned_y <- unname(table(cut(points2$y, breaks = seq(0, 1, length.out = 20))))
  binned_data2 <- rbind(binned_x, binned_y)
  
  # Equalizing the mass between point clouds seems to be necessary for good performance
  binned_data1 <- binned_data1 / sum(binned_data1)
  binned_data2 <- binned_data2 / sum(binned_data2)
  emd2d(binned_data1, binned_data2)
}
